<form action="{{ route($route) }}" class="w-100 text-light p-4 rounded" method="POST">
    ...
    {{-- trailler --}}
    <div class="mb-3">
        <label for="trailler" class="form-label">trailler</label>
        <input type="text" class="form-control bg-transparent text-light" id="trailler" name="trailler" value="{{ old('trailler') }}">
        @error('trailler') <p class="text-warning">{{ $message }}</p> @enderror
    </div>
